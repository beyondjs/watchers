...
A listener emits `all` with `(event, file)` and then the specific `add`, `change` or `unlink` with `file`, as two independent announcements: a subscriber of one that throws is reported and does not keep the other from being announced. Only those three chokidar events are forwarded; directory events are not. Events before the root's initial scan completes are suppressed, which is what the readiness of `start()` answers.

chokidar reports a change of a path at most once per 50 ms and drops a change it observes inside that window without reporting it later (`_throttle('change', path, 50)`), and it drops a raw event within 5 ms of the previous one of the same file. The service therefore examines every file it announced as added or changed once more, 75 ms after the announcement ([Recheck](../modules/service/process/watchers/recheck.ts)): when the modification time, change time, size or inode then differ from the stats chokidar reported with the announcement, it announces `change` at that moment. A file written twice within the window reaches a listener as two changes, the second at most about 75 ms after the first; a file written once is announced once, and a file removed meanwhile is left to the `unlink` chokidar reports.
